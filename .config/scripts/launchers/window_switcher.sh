...
CON_ID=$(echo "$tree" | \
    jq -r ".nodes[] 
        ...
        | $FORMAT
        | tostring" | \
    $HOME/.config/scripts/helpers/new_fuzzel.sh --dmenu --lines $count)

# Requires the actual `id` to be at the end and between paretheses
CON_ID=${CON_ID##*(}
CON_ID=${CON_ID%)}

# Get the tree again because the focus may have changed
FOCUSED_CON_ID=$(swaymsg -t get_tree | jq '.. | select(.type?) | select(.focused==true) | .id')

# Focus on the chosen window if not already in focus
if [ "$CON_ID" != "$FOCUSED_CON_ID" ]
then
